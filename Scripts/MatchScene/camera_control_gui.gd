...
func _ready():
	if camera.enabled:
		...
		_update_pivots()
		pivot.connect("item_selected",Callable(self,"_on_opt_pivot_item_selected"))
		pivot.connect("pressed",Callable(self,"_on_opt_pivot_pressed"))
		...
		btn_rot_pivot.connect("toggled",Callable(self,"_on_btn_rot_pivot_toggled"))
